        ...
        >({
            async queryFn({questionResults, learnedVocabIds}) {
                try {
                    ...
                    // 5) Calculate exp 2 points per vocab correctly answered
                    const correctCount = questionResults.filter((r) => r.isCorrect).length;
                    const expGained = correctCount * 2;

                    // Update user stats with new exp into xp_events

                    const {error: expErr} = await supabase.from("xp_events").insert({
                        user_id: profileId,
                        points: expGained,
                        source: "vocab_quiz",
                    });
...
export const {useGetRandomVocabularyQuery, useUpdateVocabsProgressMutation} = vocabApi;
